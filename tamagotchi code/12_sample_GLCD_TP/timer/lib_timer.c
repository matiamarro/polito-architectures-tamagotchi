...
extern uint32_t init_timer_total( uint8_t timer_num, uint32_t timerInterval0, uint32_t timerInterval1, uint32_t timerInterval2, uint32_t timerInterval3 ){
	  if ( timer_num == 0 )
  {
...
//*** <<< end of configuration section >>>    ***
	  ...
	  else if ( timer_num == 2 )
  {
	...
	LPC_TIM2->MCR = 7;				/* Interrupt and Reset and Stop on MR1 */
